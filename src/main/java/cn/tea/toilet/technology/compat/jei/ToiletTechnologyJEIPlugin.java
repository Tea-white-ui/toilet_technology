 ...
 *
 * 这个类是JEI集成的入口点，通过@JeiPlugin注解被JEI自动发现
 * 负责遍历所有JeiCategoryDefinition，统一注册配方类别、配方实例和配方催化剂
 *
...
public class ToiletTechnologyJEIPlugin implements IModPlugin {
    ...
    /**
     * 所有JEI配方类别定义列表
     * 新增配方类型时，只需在此列表中添加对应的Definition实现
     */
    ...
    public void registerCategories(IRecipeCategoryRegistration registration) {
        ToiletTechnology.LOGGER.info("[JEI] Registering {} recipe categories", CATEGORY_DEFINITIONS.size());
    ...
    public void registerRecipes(@NotNull IRecipeRegistration registration) {
        ...
        if (minecraft.level == null) {
            ToiletTechnology.LOGGER.warn("[JEI] Level is null, cannot register recipes");
            return;
        }
    ...
    private static <T extends Recipe<?>> void registerRecipes(
        ...
        ToiletTechnology.LOGGER.info("[JEI] Found {} recipes for type: {}", recipes.size(), definition.getRecipeType().getUid());
