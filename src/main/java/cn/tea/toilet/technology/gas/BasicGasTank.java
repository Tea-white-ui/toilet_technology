...
public final class BasicGasTank {
    ...
    public long getStored() { return stack.amount(); }
    public GasStack getStack() { return stack; }
    public boolean isEmpty() { return stack.isEmpty(); }
    public boolean isValid(GasStack candidate) { return !candidate.isEmpty() && validator.test(candidate); }
    ...
    public void deserializeNBT(CompoundTag tag) {
        ...
        if (gasId.isEmpty()) gasId = payload.getString("id");
        if (amount <= 0) amount = payload.getLong("amount");
        Gas gas = GasRegistry.byId(ResourceLocation.tryParse(gasId));
